...
class TVerClient:
    ...
    def __init__(self, logger: logging.Logger):
        ...
        try:
            ...
            self.logger.warning(
                f"Failed to create secure SSL context: {e}. Defaulting to unverified."
            )
    ...
    def get_series_episodes(self, series_id: str, series_name: str) -> List[Dict]:
        ...
        for content in contents:
            if content.get("type") == "season":
                ...
                if s_id:
                    ...
                    season_map[s_id] = traverse_obj(
                        content, ("content", "title"), default="Unknown"
                    )

        if not season_ids:
            self.logger.warning(
                f"No seasons found for series {series_name}. Trying to check if it's a single season/flat series?"
            )
            return []
        ...
        self.logger.info(
            f"Found {len(episodes)} total episodes via API for {series_name} ({counts_summary})."
        )
        return episodes
