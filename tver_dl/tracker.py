    ...
    def _get_connection(self):
        try:
            ...
            if "No route to host" in str(e) and "supabase" in self.connection_string:
                self.logger.error(
                    "Connection failed. If using Supabase, ensure you are using the Connection Pooler URL (IPv4 compliant) or have IPv6 support."
                )
            raise e

    def has_episode(self, url: str) -> bool:
        return bool(self.has_episodes_batch([url]))

    def has_episodes_batch(self, urls: list) -> set:
        if not urls:
            return set()
        try:
            with self._get_connection() as conn:
                with conn.cursor() as cur:
                    cur.execute(
                        """
                        SELECT e.episode_url FROM downloads d
                        JOIN episodes e ON d.episode_id = e.id
                        WHERE e.episode_url = ANY(%s) AND d.status = 'downloaded'
                    """,
                        (list(urls),),
                    )
                    return {row[0] for row in cur.fetchall()}
        except Exception as e:
            self.logger.error(f"Error batch-checking DB history: {e}")
            return set()
        ...
        try:
            with self._get_connection() as conn:
                with conn.cursor() as cur:
                    ...
                    tver_episode_id = (
                        ep_url.split("/")[-1]
                        if "/episodes/" in ep_url
                        else "unknown_" + ep_url[-10:]
                    )
                    ...
                    file_size = (
                        os.path.getsize(file_path) if file_path and os.path.exists(file_path) else 0
                    )
        ...
        try:
            ...
            with self._get_connection() as conn:
                with conn.cursor(cursor_factory=DictCursor) as cur:
                    ...
                    if episodes:
                        self.logger.info(
                            f"Found {len(episodes)} episodes needing subtitles in history for {tver_series_id}"
                        )
                    return episodes
        except Exception as e:
